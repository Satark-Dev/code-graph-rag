...
def _single_finding_mode_enabled() -> bool:
    raw = os.environ.get("CGR_KAFKA_SINGLE_FINDING", "0").strip().lower()
    ...
    *,
    ...
    try:
        ...
        await observability_hook.log_tool_start(
            tool_name="scoring_agent", tool_call_id=invocation
        )

        # If enabled, process exactly one finding per pipeline invocation (deterministic: first ID).
        # Otherwise preserve the previous behavior and process all IDs.
        finding_ids = (
            payload.org_tool_findings_ids[:1]
            if _single_finding_mode_enabled()
            else payload.org_tool_findings_ids
        )
        branch_name = get_branch_name_for_finding(finding_ids[0], payload.org_id)
        ...
        await repo_manager.mark_in_use(
            repo_path=target_repo_path, lease_id=repo_lease_id
        )
        ...
        request_query = {
            "findings": []
        }  # placeholder; ChatBatchService fetches finding payloads in old flow
        ...
        findings_payloads = [
            await fetch_org_tool_finding(
                org_id=payload.org_id, org_tool_findings_id=fid
            )
        ...
        run_id = invocation
        run_id, cache_key, repo_state_hash, query_payload, evidence_agent = (
            ChatOrchestratorService._prepare_context(
                request_query, target_repo_path, ingestor, run_id=run_id
            )
        )
        evidence_tool_id = str(uuid4())
        await observability_hook.log_tool_start(
            tool_name="evidence", tool_call_id=evidence_tool_id
        )
        (
            evidence_json,
            evidence_usage,
            evidence_ms,
        ) = await ChatOrchestratorService._run_evidence_stage(
            run_id=run_id,
        ...
        scoring_payload = DownstreamStagePayloadV1(
            ...
            org_tool_findings_ids=finding_ids,
