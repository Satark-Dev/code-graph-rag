...
async def process_remediation_job_message(
    *, payload: DownstreamStagePayloadV1, ingestor: Any
    ...
    try:
        ...
        await observability_hook.log_tool_start(
            tool_name="remediation", tool_call_id=payload.tool_call_id
        )
        ...
        (
            remediation_json,
            _usage_provider,
        ) = await ChatOrchestratorService._run_remediation_stage(
        ...
        input_tokens = count_tokens(API_REMEDIATION_PROMPT) + count_tokens(
            shared_payload
        )
